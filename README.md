...
Widgets use React DnD (Drag and Drop) with the following features:

- **HTML5 Backend** for native drag-and-drop
- **Smooth animations** during drag operations
- **Visual feedback** with opacity changes
- **Position preservation** - widgets maintain their order
...
### Styling
The app uses Tailwind CSS. Modify `tailwind.config.js` to customize:
- Colors
- Fonts
- Spacing
- Breakpoints
...
### Refresh Intervals
Default minimum is 5 seconds. Modify in `AddWidgetModal.tsx`:
```tsx
<input type="number" min="5" ... />
```

### Chart History
By default, charts store the last 20 data points. Modify in `WidgetCard.tsx`:
```tsx
setHistory(prev => [...prev.slice(-19), ...]) // Change -19 to adjust
```
...
- Widgets refresh independently based on their intervals
- API calls are debounced and cached
- Only visible charts render data points
...
3. Run your own backend proxy

### Data Not Updating
1. Check the API URL is correct
2. Verify the API is returning valid JSON
3. Check browser console for errors
4. Ensure refresh interval is > 0
